...
const { scanning, scanCommands, error: scanErr } = useScanner()
const { importing, importCommand, removeCommand, error: importErr } = useImport()
...
async function handleUninstall(name: string) {
  error.value = ''
  const ok = await removeCommand(name)
  if (ok) {
    localCommands.value = localCommands.value.filter(n => n !== name)
  }
  if (importErr.value) error.value = importErr.value
}
...
async function handleImportAll() {
  ...
  const targets = embeddedCommands.filter(c => !localCommands.value.includes(c.name))
  progress.value = { done: 0, total: targets.length }
  for (const cmd of targets) {
    const ok = await importCommand(cmd.name)
    if (ok && !localCommands.value.includes(cmd.name)) {
      localCommands.value.push(cmd.name)
    }
    progress.value.done++
    if (importErr.value) {
...
async function handleOpenFolder() {
  ...
  await invoke('open_folder', { path: dir + '\\commands' })
}

const available = embeddedCommands.filter(c => !localCommands.value.includes(c.name)).length
</script>
  ...
  <div>
    <div class="page-header">
      <div>
        ...
        <p class="page-desc">已安装 {{ localCommands.length }}/{{ embeddedCommands.length }} · 可导入 {{ available }}</p>
      </div>
      <div class="toolbar">
        ...
        <button class="toolbar-btn" @click="handleOpenFolder">
